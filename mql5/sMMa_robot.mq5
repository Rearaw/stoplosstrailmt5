...
void OnTick()
{
   ...
   if (current_positions < Max_Positions)
   {
      ...
      if (long_entry && cci_value < 100)  // Avoid long if overbought
      {
         if(Print_Logs) Print("Long signal detected on closed bar | Price: ", close_price, " | CCI: ", cci_value);
         ExecuteLongTrade(close_price, low_price);
      }
      if (short_entry && cci_value > -100)  // Avoid short if oversold
      {
         if(Print_Logs) Print("Short signal detected on closed bar | Price: ", close_price, " | CCI: ", cci_value);
         ExecuteShortTrade(close_price, high_price);
      }
   }
...
//| Execute long trade                                               |
//+------------------------------------------------------------------+
void ExecuteLongTrade(double signal_price, double entry_low)
{
...
//| Execute short trade                                              |
//+------------------------------------------------------------------+
void ExecuteShortTrade(double signal_price, double entry_high)
{
...
double CalculateLotSize(double entry_price, double stop_loss)
{
   ...
   // Calculate the value per lot for the stop loss distance
   double value_per_lot = (price_difference / tick_size) * tick_value;

   double lots = risk_amount / value_per_lot;

   // Round down to nearest lot step and clamp to allowed range
   lots = MathFloor(lots / lot_step) * lot_step;
   lots = MathMax(min_lot, MathMin(lots, max_lot));
